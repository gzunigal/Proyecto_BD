...
CREATE PROCEDURE `agregarEmergencia` (IN `id_user` INT, IN `id_com` INT, IN `nombre` VARCHAR(100), IN `fecha` DATE, IN `gravedad` INT, IN `estado` INT, IN `descripcion` VARCHAR(255))  NO SQL
BEGIN
...
CREATE PROCEDURE `agregarUsuario` (IN `run_user` INT, IN `id_comuna` INT(11), IN `nick_usr` VARCHAR(100), IN `name_usr` VARCHAR(100), IN `surname_usr` VARCHAR(100), IN `pass` VARCHAR(255), IN `disp` TINYINT(1), IN `administrador` TINYINT(1), IN `telefono` INT(11), IN `mail` VARCHAR(100))  NO SQL
BEGIN
